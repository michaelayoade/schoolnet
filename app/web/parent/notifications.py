"""Parent portal — notification routes."""
...
def list_notifications(
    ...
    return templates.TemplateResponse(
        "parent/notifications/list.html",
        {
            "request": request,
            "auth": auth,
...
def notification_bell(
    ...
    html = f"""<a href="/parent/notifications" class="relative p-2 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/50">
...
def mark_notification_read(
    ...
    return RedirectResponse(
        url="/parent/notifications?success=Notification+marked+as+read",
        status_code=303,
    )
