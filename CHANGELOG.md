...
- [Security] `delete_avatar()` in `app/services/avatar.py` now uses `.resolve()` path bounds check — naive string-replacement path construction replaced with the same `_safe_asset_path()` pattern used in `branding_assets.py`, preventing path-traversal deletion outside the avatar directory (PR #69)
- [Security] `generate_css()` in `app/services/branding.py` now sanitises font family names and hex colour values — font-family strings stripped of all characters outside `[A-Za-z0-9 _-]`; colour values validated against `^#[0-9a-fA-F]{3,6}$` before CSS interpolation, preventing CSS injection via crafted branding settings (PR #69)
- [Security] SMTP password now resolved through `resolve_secret()` in `app/services/email.py` — `_get_smtp_config()` previously read `SMTP_PASSWORD` from the environment directly, bypassing the vault/OpenBao reference resolution used for other secrets such as JWT and TOTP keys (PR #67)
- [Security] Bare `except Exception:` in `FileUploadService.delete()` replaced with `except (OSError, IOError, ValueError) as exc:` — exception detail now included in the `logger.warning()` call; catches only expected storage-layer errors rather than masking all exceptions (`app/services/file_upload.py`) (PR #67)
- [Security] `_refresh_cookie_secure()` now recognises `'on'` as a truthy value in addition to `'true'`, `'1'`, and `'yes'` — environment values set by some hosting providers were not activating the secure flag (`app/services/auth_flow.py`) (PR #62)
- [Security] HTML injection in password-reset email fully landed — `html.escape()` applied to `person_name` and `reset_link` before interpolation into the HTML body in `send_password_reset_email()` (`app/services/email.py`) (PR #62)
...
### Fixed
- [Fixed] `app/api/__init__.py` and `app/schemas/__init__.py` re-added as regression fix — PR #26 batch-deps-c4-config changes had not landed on `main`; both namespace package init files are now present, consistent with all other `app/` sub-packages (PR #65)
- [Fixed] `python-multipart` declared as explicit runtime dependency in `pyproject.toml` — was an undeclared transitive dependency despite being required by `UploadFile` and `Form()` across 15 files; deployments with stripped transitive deps would crash on startup (`pyproject.toml`) (PR #65)
- [Fixed] OpenTelemetry instrumentation packages re-upgraded from pre-release `0.47b0` to stable as regression fix — PR #26 batch-deps-c4-config changes had not landed on `main`; `opentelemetry-instrumentation-fastapi`, `-sqlalchemy`, and `-celery` now pinned to stable releases (`pyproject.toml`) (PR #66)
- [Fixed] `types-cachetools` and `types-redis` re-added to dev dependencies as regression fix — PR #26 changes had not landed on `main`; mypy can now type-check `cachetools.TTLCache` (rate limiter) and Redis client calls without `ignore_missing_imports` workarounds (`pyproject.toml`) (PR #66)
- [Fixed] `import json` moved from inside `paystack_webhook()` function body to module-level import block — inline import was a code quality regression that survived cycle-6 and cycle-7 fixes (`app/api/payments.py`) (PR #66)
- [Fixed] Alembic migration chain repaired — `003_file_uploads` `down_revision` corrected from `002_billing_schema` to `002_billing`, allowing `alembic upgrade head` to succeed on fresh databases (`alembic/versions/003_file_uploads.py`) (PR #62)
...
### Changed
- [Changed] Docker Compose services renamed from `starter_template_*` to `schoolnet_*` and app port changed from `8001` to `8006` to match project identity (`docker-compose.yml`) (PR #62)
- [Changed] Regression fixes from cycle-6 confirmed and applied — remaining `db.query()` calls in `app/api/auth_flow.py` migrated to SQLAlchemy 2.0 `select()`/`scalars()`; pagination (`limit`/`offset`) confirmed present on `GET /applications/my` and `GET /applications/school/{id}`; inline imports in `app/api/applications.py` and `app/api/payments.py` moved to module top-level (PR #63)
- [Changed] Inline ownership DB queries moved out of `app/api/admission_forms.py` route handlers into the service layer — `AdmissionFormService.assert_school_owner(school_id, person_id)` added; `create_form()`, `update_form()`, and `close_form()` handlers now call the service method and catch `PermissionError` → `HTTPException(403)` (CLAUDE.md Rule #1) (PR #61)
- [Changed] Inline IDOR ownership checks moved out of `app/api/applications.py` route handlers into the service layer — `ApplicationService.assert_viewer_access(application, person_id, roles)` and `ApplicationService.assert_reviewer_access(application, person_id, roles)` added; `get_application()` and `review_application()` catch `PermissionError` → `HTTPException(403)` (CLAUDE.md Rule #1) (PR #58)
- [Changed] Missing `response_model` declarations added to 4 API endpoints — `POST /applications/purchase` → `PurchaseResponse(checkout_url, reference, application_id)`; `POST /notifications/me/read-all` → `MarkedReadResponse(marked_read)`; `POST /scheduler/tasks/refresh` → `ScheduleRefreshResponse`; `POST /scheduler/tasks/{task_id}/enqueue` → `TaskEnqueueResponse` (`app/api/applications.py`, `app/api/notifications.py`, `app/api/scheduler.py`; schemas in `app/schemas/`) (PR #59)
- [Changed] Pagination added to `GET /schools/my` and `GET /admission-forms/school/{id}` — `limit` and `offset` query parameters prevent unbounded result sets; `GET /schools/my` defaults limit=50 (max 200), `GET /admission-forms/school/{id}` defaults limit=100 (max 500) (`app/api/schools.py`, `app/api/admission_forms.py`) (PR #60)
