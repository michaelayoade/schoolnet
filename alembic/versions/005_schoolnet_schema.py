...
def upgrade() -> None:
    ...
    for enum_name, values in [
        ...
        (
            "schooltype",
            ("primary", "secondary", "primary_secondary", "nursery", "nursery_primary"),
        ),
    ...
    if not inspector.has_table("admission_forms"):
        op.create_table(
            ...
            sa.Column(
                "price_id", UUID(as_uuid=True), sa.ForeignKey("prices.id"), index=True
            ),
    ...
    if not inspector.has_table("applications"):
        op.create_table(
            ...
            sa.Column(
                "admission_form_id",
                UUID(as_uuid=True),
                sa.ForeignKey("admission_forms.id"),
                nullable=False,
                index=True,
            ),
            sa.Column(
    ...
    if not inspector.has_table("ratings"):
        op.create_table(
            ...
            sa.Column("updated_at", sa.DateTime(timezone=True)),
            sa.UniqueConstraint(
                "school_id", "parent_id", name="uq_ratings_school_parent"
            ),
        )
